...
export async function POST(request: Request) {
  ...
  if (!to) return xml(twimlVoiceSay("This Yumna number is not configured."), 400);
  ...
  if (!identity) {
    return xml(twimlVoiceSay("This Yumna number is not linked to a verified workspace."), 404);
  }
