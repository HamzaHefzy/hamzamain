    ...
    <div className="operator-page">
      <header className="operator-page-header">
        <div>
          ...
          <p>Subscriptions keep Yumna aligned to you—not advertisers or merchants—and control execution capacity directly.</p>
        </div>
        <div className="operator-billing-header-actions">
          <div className="operator-status-pill status-completed">
            {(entitlement.plan === "operator" ? "Yumna" : entitlement.plan.replaceAll("_", " ")) + " · " + entitlement.status}
          </div>
      ...
      <section className="operator-metrics">
        ...
        <article><span>Current plan</span><strong className="operator-plan-name">{entitlement.plan === "operator" ? "Yumna" : entitlement.plan.replaceAll("_", " ")}</strong><small>{entitlement.status}</small></article>
