...
export default function Logo({ className = "", markOnly = false }: LogoProps) {
  return (
    <span className={`operator-logo ${className}`.trim()} aria-label="Operator">
      <svg className="operator-logo-mark" viewBox="0 0 48 40" role="img" aria-hidden="true" fill="none">
      ...
      {!markOnly && <span className="operator-logo-word">Operator</span>}
    </span>
  );
}
