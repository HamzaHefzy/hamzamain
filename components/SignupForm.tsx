...
export default function SignupForm() {
  ...
  return (
    <form className="auth-form" onSubmit={submit}>
      ...
      <label><span>Workspace name <small>Optional</small></span><input name="workspaceName" placeholder="e.g. Hamza's Yumna" /></label>
      {error ? <div className="form-error" role="alert">{error}</div> : null}
      <button className="m-button dark auth-submit" type="submit" disabled={busy}>
        {busy ? "Creating workspace…" : "Create Yumna workspace"}
      </button>
    </form>
  );
}
